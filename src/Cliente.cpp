...
while(aux.leerDeDisco(pos++)){
    ...
    for(int i=0;i<cant;i++){
        if(vec[i].getFecha().getMes()==mes && vec[i].getFecha().getAnio()==anio){
       if(vec[i].getIDC()==aux.getIDP()){
            id=vec[i].getID();
            cantidadProductos+=contarCantidad(id);
        }
